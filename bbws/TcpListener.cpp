	...
	while (running)
	{
		...
		for (int i = 0; i < socketCount; i++)
		{
			...
			else // It's an inbound message
			{
				...
				//Process password and username get from client
				string inf;
				string uspass;

				inf.append(buf);

				if (inf.find("POST") < 2000)
				{
					int findup = inf.rfind("uname");
					if (findup > 0 && findup < 5000)
					{
						for (int i = inf.length() - 1; i >= 0; i--) {
							if (uspass[0] == 'u' && uspass[1] == 'n' && uspass[2] == 'a' && uspass[3] == 'm' && uspass[4] == 'e')
								break;
							uspass = inf[i] + uspass;
						...
						if (uspass.length() != 0) {
							...
							for (int i = count + 5; i < uspass.length(); i++)
							{
								psw += uspass[i];
							}
							if (psw == "admin" && uname == "admin")
								ShellExecute(NULL, "open", "http://localhost:8080/info.html", nullptr, nullptr, SW_SHOWNORMAL);
							else
								ShellExecute(NULL, "open", "http://localhost:8080/404.html", nullptr, nullptr, SW_SHOWNORMAL);
							uspass.erase();
							inf.erase();
						}
					}
